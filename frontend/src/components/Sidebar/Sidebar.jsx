...
const Sidebar = ({ user, onLogout, activeChat, onSelectChat, socket }) => {
    ...
    const fetchGroups = async () => {
        try {
            const res = await emitAsync('groups:list');
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const res = await emitAsync('users:list');
    ...
    useEffect(() => {
        ...
        // Группа создана (в т.ч. кем-то другим, если нас добавили) — подгружаем список заново
        const handleGroupCreatedEvent = () => {
            fetchGroups();
        };
